...
<script>
	...
	export default {
		...
		computed: {
			...
			customTitle: {
				get () { return this.$store.state.curriculum.projet.customTitle },
				set (value) { this.$store.state.curriculum.projet.customTitle = value }
			},

			description: {
				get () { return this.$store.state.curriculum.projet.description },
				set (value) { this.$store.state.curriculum.projet.description = value }
			},
		...
		i18n: {
			messages: {
				...
				'fr': {
					presentation: 'Projet pro',
					presentationHelpText: 'Rédigez une présentation concerant votre projet pro.',
					placeholder: 'Rédigez une présentation concerant votre projet pro.'
				}
			}
		}
	}
